...
threedgut::Status threedgut::GUTRenderer::renderForward(const RenderParameters& params,
...

    {
        ...
        const uint32_t numBlocks = virtual_tiles_total;
        
        LOG_INFO(m_logger, "Fine-grained load balancing: virtualTiles=%u, numBlocks=%u, threadsPerBlock=128", 
                    virtual_tiles_total, numBlocks);
        
        ::renderFineGrainBalanced<<<numBlocks, 128, 0, cudaStream>>>( // 128 threads = 4 warps * 32 threads
            params,
