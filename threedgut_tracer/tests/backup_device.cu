...
#pragma unroll
    ...
    template <typename TRay>
    // Fine-grained warp-level处理函数 - 基于Algorithm 3优化：批量合并加载 + 4warps共享处理
    static inline __device__ void evalFineGrainedWarpShared(
        const threedgut::RenderParameters& params,
                                       TRay& ray,
                                       const tcnn::uvec2* __restrict__ sortedTileRangeIndicesPtr,
                                       const uint32_t* __restrict__ sortedTileParticleIdxPtr,
        ...
        // 🚀 Algorithm 3 核心：Block级共享内存，供4个warps共同使用  
        constexpr uint32_t BATCH_SIZE = 128;
        __shared__ PrefetchedParticleData batchSharedParticles[BATCH_SIZE];
        
        // 使用原始16x16 tile的粒子数据，4个warps处理4个不同pixels
        const uint32_t tileIdx = tile.y * tileGrid.x + tile.x;
        const tcnn::uvec2 tileParticleRangeIndices = sortedTileRangeIndicesPtr[tileIdx];
        ...
        // 🎯 批量流水线处理：边加载边计算，4个warps共享粒子数据
        for (uint32_t batchStart = 0; batchStart < tileNumParticlesToProcess; batchStart += BATCH_SIZE) {
            
            // === 阶段1：128线程协作加载 - 死ray也必须参与避免死锁 ===
            uint32_t loadIdx = threadIdx.x; // 0-127
            uint32_t globalParticleIdx = batchStart + loadIdx;
            
            // 🔑 关键：即使ray死了，也要参与数据加载（为其他warps服务）
            if (loadIdx < BATCH_SIZE && globalParticleIdx < tileNumParticlesToProcess) {
                const uint32_t toProcessSortedIndex = tileParticleRangeIndices.x + globalParticleIdx;
                const uint32_t particleIdx = sortedTileParticleIdxPtr[toProcessSortedIndex]; // 🎯 合并访问！
                
                if (particleIdx != GUTParameters::InvalidParticleIdx) {
                    batchSharedParticles[loadIdx] = {particleIdx, particles.fetchDensityParameters(particleIdx)};
                } else {
                    batchSharedParticles[loadIdx].idx = GUTParameters::InvalidParticleIdx;
                }
            } else if (loadIdx < BATCH_SIZE) {
                batchSharedParticles[loadIdx].idx = GUTParameters::InvalidParticleIdx;
            }
            
            __syncthreads(); // 死ray也必须等待数据加载完成
            
            // === 阶段2：每个warp处理自己的ray，但遍历相同的粒子 ===
            uint32_t batchParticleCount = min(BATCH_SIZE, tileNumParticlesToProcess - batchStart);
            uint32_t alignedBatchCount = ((batchParticleCount + 31) / 32) * 32;
            
            for (uint32_t j = laneId; j < alignedBatchCount; j += 32) {
                
                // ✅ 死ray快速跳过，但仍参与外层循环（为了同步）
                if (!ray.isAlive()) continue; // 用continue而不是break！
                ...
                // 4个warps都处理相同的粒子j，但用各自的ray参数
                if (j < batchParticleCount) {
                    const PrefetchedParticleData& particleData = batchSharedParticles[j];
                    
                    if (particleData.idx != GUTParameters::InvalidParticleIdx) {
                        // 每个warp用自己的ray计算intersection
                        if (particles.densityHit(ray.origin, ray.direction, 
                            ...
                            // 获取特征
                            if constexpr (Params::PerRayParticleFeatures) {
                                hitFeatures = particles.featuresFromBuffer(particleData.idx, ray.direction);
                            } else {
                ...
                // 🔑 早停只影响当前batch，死ray仍参与下一batch的加载
                if (should_terminate) break; // 跳出当前batch，但继续外层循环
            }
            __syncthreads();
        }
    }
